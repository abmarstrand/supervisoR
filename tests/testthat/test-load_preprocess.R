...
test_that("load_default_data loads data correctly", {
  ...
  expect_true(all(c("gene_sets", "mapping", "pathway_graph", "relations") %in% names(data)))
...
test_that("load_and_preprocess_gene_sets works with and without translation_layer", {
  ...
  expect_true(all(c("gene_sets", "mapping", "pathway_graph") %in% names(result_no_translation)))
