...
## Plotting a subgraph with a lollipop plot, including significances

By changing the shape of the points in the lollipop plot, we can also plot whether a given pathway is significance in a given condition. Here, we provide random p-values. The p-value dataframe must be formatted in the same way as the enrichment score dataframe, including where pathway names are located. The cutoff for significance can be changed, but defaults to 0.05.
```{r}
significance_scores <- data.frame(Condition.1 = sample(seq(0,1,by = 0.0001),length(default_data$gene_sets)),
                                Condition.2 = sample(seq(0,1,by = 0.0001),length(default_data$gene_sets)),
                                Condition.3 = sample(seq(0,1,by = 0.0001),length(default_data$gene_sets)),
                                Condition.4 = sample(seq(0,1,by = 0.0001),length(default_data$gene_sets)))
rownames(significance_scores) <- names(default_data$gene_sets)
