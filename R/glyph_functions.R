...
#' Generates a cache of glyph images across all conditions. 
#'
#' Generates glyph images for each pathway and caches them to improve performance in the shiny app.
#' Note that this is very slow, exponentially more so if your graph and number of comparisons are large. For n comparisons, this will result in:
#' 
#' \deqn{\text{Total combinations} = n \cdot (2^{n-1} - 1)}
#' 
#' Since the function takes more than a minute per combination for the full REACTOME dataset, this rapidly becomes extremely slow. For reference, this takes >30 minutes for three conditions on a regular desktop.
#' The function was designed for use when you want to host a permanent shiny app exhibiting your data, and want to do most of the heavy lifting yourself.
#' This function is not necessary for the functioning of the package. 
#' 
#'
#' @importFrom base64enc dataURI
#' @importFrom digest digest
...
#' @return A named list where each name corresponds to a pathway and contains the base64-encoded image URI.
#' @export
pregenerate_all_possible_glyphs <- function(
    g,
    gene_sets,
    enrichment_scores,
    mapping,
  ...
  ### GENERATE ALL POSSIBLE NON-REFERENCE GLYPHS
  enrichment_scores <- enrichment_scores[rownames(enrichment_scores) %in% V(g)$label,]
  g <- induced_subgraph(g, vids = which(V(g)$label %in% rownames(enrichment_scores)))
  combinations <- unlist(lapply(1:length(colnames(enrichment_scores)),
                                function(k) combn(colnames(enrichment_scores), k, simplify = FALSE)),recursive = F)
  for (i in combinations) {
    writeLines(paste0("Processing combination of: ", paste(i, collapse = " ")))
    enriched_subset <- enrichment_scores[, i, drop = FALSE]
    data_hash <- digest::digest(list(enrichment_scores = enriched_subset,
                                     conditions = i), algo = "md5")
    if (is.null(enriched_subset)) {
      enrichment_limits <- NULL
    }
    enrichment_limits <- c(min(enriched_subset, na.rm = TRUE), max(enriched_subset, na.rm = TRUE))
    glyphs <- generate_glyph_images_cached(
      g = g,
      gene_sets = gene_sets,
      enrichment_scores = enriched_subset,
      conditions = i,
      mapping = mapping,
      enrichment_limits = enrichment_limits,
      glyph_size = glyph_size,
    ...
    ### ALSO GENERATE ALL REFERENCE GLYPHS
    if (!length(i)==1) {
      for (zz in i) {
        ref_scores <- enriched_subset[, zz, drop = FALSE]
        # Subtract the reference comparison scores from all selected comparisons
        enriched_subset_ref <- sweep(enriched_subset, 1, ref_scores[, 1], FUN = "-")
        combinations_ref <- unlist(lapply(1:length(colnames(enriched_subset_ref[,!colnames(enriched_subset_ref) == zz, drop = F])),
                                          function(k) combn(colnames(enriched_subset_ref[,!colnames(enriched_subset_ref) == zz, drop = F]), k, simplify = FALSE)),
                                   recursive = F)
        
        for (xx in combinations_ref) {
          writeLines(paste0("\tProcessing reference ", zz, " against: ", paste(xx, collapse = " ")))
          enriched_subset_ref_xx <- enriched_subset_ref[, xx, drop = FALSE]
          
          data_hash <- digest::digest(list(enrichment_scores = enriched_subset_ref_xx,
                                           conditions = xx), algo = "md5")
          if (is.null(enriched_subset_ref_xx)) {
            enrichment_limits <- NULL
          }
          enrichment_limits <- c(min(enriched_subset_ref_xx, na.rm = TRUE), max(enriched_subset_ref_xx, na.rm = TRUE))
          glyphs <- generate_glyph_images_cached(
            g = g,
            gene_sets = gene_sets,
            enrichment_scores = enriched_subset_ref_xx,
            conditions = xx,
            mapping = mapping,
            enrichment_limits = enrichment_limits,
            glyph_size = glyph_size,
